...
KafkaClient.prototype.connectToBroker = function(broker, callback) {
  ...
  logger.debug(
    `Trying to connect to host: ${broker.host} port: ${broker.port}`
  );
  ...
  const brokerWrapper = this.setupBroker(
    broker.host,
    broker.port,
    false,
    this.brokers
...
KafkaClient.prototype.getController = function(callback) {
  ...
  this.loadMetadata((error, result) => {
    ...
    if (
      !result[1].clusterMetadata ||
      result[1].clusterMetadata.controllerId == null
    ) {
      return callback(
        new errors.BrokerNotAvailableError("Controller broker not available")
      );
    }
    ...
    var broker = this.getBroker(
      controllerMetadata.host,
      controllerMetadata.port
    );
...
KafkaClient.prototype.refreshBrokerMetadata = function(callback) {
  ...
  async.waterfall(
    [
      callback => this.getAvailableBroker(callback),
      (broker, callback) => this.loadMetadataFrom(broker, callback)
    ],
    (error, result) => {
      ...
      if (error) {
        ...
        return this.emit(
          "error",
          new NestedError("refreshBrokerMetadata failed", error)
        );
      }
      this.updateMetadatas(result, true);
      this.refreshBrokers();
...
KafkaClient.prototype.loadMetadataFrom = function(broker, cb) {
  ...
  var request = protocol.encodeMetadataRequest(
    this.clientId,
    correlationId,
    []
  );
  ...
  this.sendWhenReady(
    ...
    correlationId,
    request,
    protocol.decodeMetadataResponse,
    cb
  );
};
...
KafkaClient.prototype.setBrokerMetadata = function(brokerMetadata) {
  ...
  if (
    !_.isEmpty(oldBrokerMetadata) &&
    !_.isEqual(oldBrokerMetadata, brokerMetadata)
  ) {
...
KafkaClient.prototype.updateMetadatas = function(
  metadatas,
  replaceTopicMetadata
) {
  assert(
    metadatas && Array.isArray(metadatas) && metadatas.length === 2,
...
KafkaClient.prototype.wrapTimeoutIfNeeded = function(
  socketId,
  correlationId,
  callback,
  overrideTimeout
...
KafkaClient.prototype.getApiVersions = function(broker, cb) {
  if (!broker || !broker.isConnected()) {
    return cb(
      new errors.BrokerNotAvailableError(
        "Broker not available (getApiVersions)"
      )
    );
  }
  ...
  async.mapValuesLimit(
    ...
    (brokerMetadata, brokerId, cb) => {
      ...
      if (!broker || !broker.isConnected()) {
        return cb(
          new errors.BrokerNotAvailableError(
            "Broker not available (getListGroups)"
          )
        );
      }
      ...
      async.mapValuesLimit(
        ...
        (groups, coordinator, cb) => {
          ...
          if (!broker || !broker.isConnected()) {
            return cb(
              new errors.BrokerNotAvailableError(
                "Broker not available (getDescribeGroups)"
              )
            );
          }

          const correlationId = this.nextId();
          const request = protocol.encodeDescribeGroups(
            this.clientId,
            correlationId,
            groups
          );
...
KafkaClient.prototype.initializeBroker = function(broker, callback) {
  if (!broker || !broker.isConnected()) {
    return callback(
      new errors.BrokerNotAvailableError(
        "Broker not available (initializeBroker)"
      )
    );
  }
  ...
  this.getApiVersions(broker, (error, versions) => {
    if (error) {
      if (error instanceof TimeoutError) {
        logger.debug(
          "getApiVersions request timedout probably less than 0.10 using base support"
        );
    ...
    if (_.isEmpty(versions)) {
      return callback(
        new Error(`getApiVersions response was empty for broker: ${broker}`)
      );
    }
...
KafkaClient.prototype.saslAuth = function(broker, callback) {
  ...
  const apiVersion = broker.apiSupport
    ? broker.apiSupport.saslHandshake.usable
    : undefined;
  if (typeof apiVersion !== "number") {
    callback(
      new errors.SaslAuthenticationError(
        null,
        "Broker does not support SASL authentication"
      )
    );
    return;
  }
  ...
  async.waterfall(
    [
      callback => {
        logger.debug(
          `Sending SASL/${mechanism} handshake request to ${broker}`
        );

        const correlationId = this.nextId();
        const request = protocol.encodeSaslHandshakeRequest(
          this.clientId,
          correlationId,
          apiVersion,
          mechanism
      ...
      (enabledMechanisms, callback) => {
        logger.debug(
          `Sending SASL/${mechanism} authentication request to ${
            broker.socket.addr
          }`
        );
        ...
        const correlationId = this.nextId();
        const request = protocol.encodeSaslAuthenticateRequest(
          this.clientId,
          correlationId,
          apiVersion,
          auth
...
KafkaClient.prototype.createBroker = function(host, port, longpolling) {
  logger.debug("createBroker", host, port);
  var self = this;
  var socket;
  if (self.ssl) {
    ...
    if (!hadError && self.closing) {
      ...
      if (!error) {
        if (self.options.sasl && socket.saslAuthCorrelationId !== undefined) {
          ...
          const message =
            "Broker closed connection during SASL auth: bad credentials?";
          error = new errors.SaslAuthenticationError(null, message);
        } else {
          error = new errors.BrokerNotAvailableError(
            "Broker not available (socket closed)"
          );
        }
      }
  ...
  function retry(s) {
    ...
    s.retryTimer = setTimeout(function() {
      ...
      if (brokerWrapper.isIdle()) {
        logger.debug(
          `${self.clientId} to ${socket.addr} is idle not reconnecting`
        );
        s.closing = true;
...
KafkaClient.prototype.loadMetadataForTopics = function(topics, callback) {
  ...
  if (!broker || !broker.isConnected()) {
    return callback(
      new errors.BrokerNotAvailableError(
        "Broker not available (loadMetadataForTopics)"
      )
    );
  }
  ...
  async.series(
    [
      ...
      cb => {
        ...
        const request = supportedCoders.encoder(
          this.clientId,
          correlationId,
          topics
        );
...
KafkaClient.prototype.createTopics = function(topics, callback) {
  ...
  this.sendControllerRequest(
    "createTopics",
    [topics, this.options.requestTimeout],
    callback
  );
};
...
KafkaClient.prototype.sendRequest = function(request, callback) {
  ...
  const sendToBroker = async.ensureAsync((payload, leader, callback) => {
    ...
    if (!broker || !broker.isConnected()) {
      ...
      callback(
        new errors.BrokerNotAvailableError("Broker not available (sendRequest)")
      );
      return;
    }
    ...
    const encoder =
      request.data.args != null
        ? coder.encoder.apply(null, request.data.args)
        : coder.encoder;
    const decoder =
      request.data.decoderArgs != null
        ? coder.decoder.apply(null, request.data.decoderArgs)
        : coder.decoder;
 ...
 */
KafkaClient.prototype.sendRequestToBroker = function(
  brokerId,
  requestType,
  args,
  callback
...
KafkaClient.prototype.leaderLessPayloads = function(payloads) {
  return _.filter(
    payloads,
    payload => !this.hasMetadata(payload.topic, payload.partition)
  );
};
...
KafkaClient.prototype.verifyPayloadsHasLeaders = function(payloads, callback) {
  ...
  logger.debug(
    "payloads has no leaders! Our metadata could be out of date try refreshingMetadata",
    leaderLessPayloads
  );
  this.refreshMetadata(_.map(leaderLessPayloads, "topic"), error => {
    ...
    if (payloadWithMissingLeaders.length) {
      logger.error(
        "leaders are still missing for %j",
        payloadWithMissingLeaders
      );
...
KafkaClient.prototype.wrapControllerCheckIfNeeded = function(
  requestType,
  requestArgs,
  callback
) {
...
KafkaClient.prototype.sendControllerRequest = function(
  requestType,
  args,
  callback
) {
  this.getController((error, controller, controllerId) => {
    ...
    callback = this.wrapControllerCheckIfNeeded(
      requestType,
      originalArgs,
      originalCallback
    );
...
KafkaClient.prototype.sendFetchRequest = function(
  ...
  function stateValidator(unused, type, message) {
    ...
    if (
      ...
      const causeForError =
        `consumer.closing: ${consumer.closing} ` +
        `consumer.connecting: ${consumer.connecting} ` +
        `consumer.rebalancing: ${consumer.rebalancing} ` +
        `consumer.memberId !== memberId: ${consumer.memberId !== memberId} ` +
        `consumer.generationId !== generationId: ${consumer.generationId !==
          generationId}`;

      logger.error(
        "ignoring message due to it being from an old group - memberId: " +
          memberId,
        "!=" +
          consumer.memberId +
          " - generationId: " +
      ...
      callback => {
        const request = {
          ...
          data: {
            ...
            decoderArgs: [
              this._createMessageHandler(consumer, stateValidator),
              maxTickMessages
            ]
          }
        };
...
KafkaClient.prototype.sendWhenReady = function(
  broker,
  correlationId,
  request,
  decode,
...
KafkaClient.prototype.sendProduceRequest = function(
  payloads,
  requireAcks,
  ackTimeoutMs,
  callback
  ...
  async.series(
    ...
    (err, result) => {
      if (err) {
        if (
          err.message === "NotLeaderForPartition" ||
...
KafkaClient.prototype.describeConfigs = function(payload, callback) {
  ...
  _.forEach(payload.resources, function(resource) {
    if (resourceTypeMap[resource.resourceType] === undefined) {
      err = new Error(
        `Unexpected resource type ${resource.resourceType} for resource ${
  ...
  async.mapValuesLimit(
    ...
    (brokerMetadata, brokerId, cb) => {
      ...
      if (!broker || !broker.isConnected()) {
        return cb(
          new errors.BrokerNotAvailableError(
            "Broker not available (describeConfigs)"
          )
        );
      }
      ...
      const request = protocol.encodeDescribeConfigsRequest(
        this.clientId,
        correlationId,
        payload,
        apiVersion
      ...
      this.sendWhenReady(
        ...
        correlationId,
        request,
        protocol.decodeDescribeConfigsResponse(apiVersion),
        cb
      );
    },
    (err, results) => {
